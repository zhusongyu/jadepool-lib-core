...
const ORDER_BIZTYPES = {
  ...
  // 非资产流转订单
  SYSTEM_CALL: 'SYSTEM_CALL', // 出：区块链函数调用订单，(value为消耗为0，fee消耗, 额外业务数据在action中记录)
  DELEGATE: 'DELEGATE', // 出：抵押订单，(value为抵押值，fee消耗，抵押用可收回)
  PRINCIPAL_FUND: 'PRINCIPAL_FUND', // 入：已回收的抵押额，(value为本金，fee为0，属于本金收回)
  INTEREST_FUND: 'INTEREST_FUND' // 入：收益记录订单，(value为收益额，fee为0, 属于额外利息收入)
}
